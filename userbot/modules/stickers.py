...
async def kang(event):
            ...
            while x.text == PACK_FULL:
                ...
                if x.text == "Invalid pack selected.": # That pack doesn't exist
                    ...
                    await event.edit(
                        ...
                        f"{emoji} emojisi ile birlikde! "
                        f"Paketı `[burada](t.me/addstickers/{packname})`mövcuddur.`",
                        parse_mode='md')
                    return
    ...
    await event.edit(
        f"`Sticker {number}{'(animasyonlu)' if is_anim else ''} sayılı paketə əlavə edildi, "
        f"{emoji} emoji ilə! "
        f"Paketi `[burada](t.me/addstickers/{packname})` mövcuddur.`",
        parse_mode='md')
...
CmdHelp('stickers').add_command(
    ...
    'dızla', '<emoji(lər)> <numara>', 'Göstərilən paketə etiket və ya şəkil əlavə edir və emoji etiketinin emoji olaraq istifadə olunur.'
).add()
