...
async def cevir(event):
    ...
    if islem == "foto" or islem == "[sş][eə]kil":
    ...
    elif islem == "ses" or islem == "səs":
        EFEKTLER = ["usaq", "robot", "qeribe", "suretli", "heserat", "seda"]
        # https://www.vacing.com/ffmpeg_audio_filters/index.html #
        KOMUT = {"usaq": '-filter_complex "rubberband=pitch=1.5"', "robot": '-filter_complex "afftfilt=real=\'hypot(re,im)*sin(0)\':imag=\'hypot(re,im)*cos(0)\':win_size=512:overlap=0.75"', "qeribe": '-filter_complex "acrusher=level_in=8:level_out=18:bits=8:mode=log:aa=1"', "suretli": "-filter_complex \"rubberband=tempo=1.5\"", "heserat": '-filter_complex "afftfilt=real=\'hypot(re,im)*cos((random(0)*2-1)*2*3.14)\':imag=\'hypot(re,im)*sin((random(1)*2-1)*2*3.14)\':win_size=128:overlap=0.8"', "seda": "-filter_complex \"aecho=0.8:0.9:500|1000:0.2|0.1\""}
...
CmdHelp('cevir').add_command(
    ...
    'çevir gif', '<cavab>', 'Videoyu veya animasyonlu stickeri gife çevirir.'
).add_command(
    'çevir ses', '<cavab> <usaq/robot/qeribe/suretli/heserat/seda>', 'Sese efekt uygular.'
).add_command(
    'çevir mp3', '<cavab>', 'Cavab verdiyiniz videonu mp3-ə çevirər.'
).add()
