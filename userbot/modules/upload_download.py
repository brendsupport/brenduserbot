""" Serverə fayl endirmə/yükləmə edəcəyiniz bir UserBot moduludur. """
...
def humanbytes(size):
    """ Boyut oxunacaq vəziyyətdə olması üçün bayt olaraq göstərili """
...
def time_formatter(milliseconds: int) -> str:
    """ Daha gözəl görünməsi üçün zamanı milisaniyə olaraq göstərər. """
    ...
    tmp = ((str(days) + f" {LANG['DAY']}, ") if days else "") + \
        ...
        ((str(milliseconds) + " milisaniyə, ") if milliseconds else "")
    return tmp[:-2]


@register(pattern=r".yukle(?: |$)(.*)", outgoing=True)
async def download(target_file):
    """ .yukle əmri ilə userbot serverinə fayl yükləyə bilərsiniz . """
...
@register(pattern=r".uploader (.*)", outgoing=True)
async def uploadir(udir_event):
    """ .uploader əmri bir qovluqdakı bütün faylları uploadlamanıza kömək edər """
...
async def upload(u_event):
    """ .upload əmri userbot serverindən fayl uploadlamaya kömək edər. """
...
def extract_w_h(file):
    """ Bir medyanın yüksəkliyini-genişliyini göstərər. """
    command_to_run = [
        "ffprobe",
        "-v",
        "quiet",
...
async def uploadas(uas_event):
    """ .uploadas əmri upload edərkən bəzı problemləri görməyinizə kömək edər. """
    await uas_event.edit("Biraz gözləyin...")
    ...
    if os.path.exists(file_name):
        ...
        try:
            if supports_streaming:
                ...
                await uas_event.edit("TBD: Hal hazırda uyğulanamadı.")
                return
            os.remove(thumb)
...
CmdHelp('updown').add_command(
    'download', '<bağlantı-fayl adı> (ya da bir şeyə cavab verərək)', 'Severə fayl yükləyər.'
).add_command(
    'upload', '<serverdəki fayl yolu>', 'Serverinizdə bir faylı söhbətə upload edər.'
).add_command(
    'wupload', ' <fayla cavab verin> anonfiles|transfer|filebin|tmpninja|anonymousfiles|megaupload|bayfiles|letsupload|vshare', 'Seçtiyiniz websaytına yükləyər.'
).add_command(
    'unzip', '<yanıt>', 'Cavab verdiyiniz Zip faylını çıxarar.'
).add_command(
    'uploadir', '<klasör>', 'Bütün qovluğu yükləyər.'
).add()
