...
async def disapprovepm(disapprvpm):
    ...
    if BOTLOG:
        await disapprvpm.client.send_message(
            ...
            " Baş nazirlik icazəsi qaldırıldı.",
        )
...
async def blockpm(block):
    if block.reply_to_msg_id:
        ...
        if replied_user.id in BRAIN_CHECKER or replied_user.id in WHITELIST:
            await block.edit(
                "`Kişi yox! Brend menecerini bloklamayacam!!`"
            )
            return
...
async def unblockpm(unblock):
    ...
    if BOTLOG:
        await unblock.client.send_message(
            ...
            " şəxsinin blokundan çıxarıldı.",
        )

CmdHelp('pmpermit').add_command(
    'approve', None, 'Cavab verilmiş istifadəçinin PM-yə icazəsi.', 
).add_command(
    'disapprove', None, 'Cavab verən istifadəçinin PM təsdiqini silir.'
).add_command(
    'block', '<istifadəçi adı/yanıtlama>', 'İstifadəçinin qarşısını alır.'
).add_command(
    'unblock', '<istifadəçi adı/yanıtlama>', 'İstifadəçini blokdan çıxarır.'
).add_command(
